...
-- a trigger on the products table.
-- Includes a table to store alternative shipping locations for each customer
-- Developed using MySql v5.7; MySql Workbench v8.0
...
DROP PROCEDURE IF EXISTS uspAddCustomerOrders;
...
DROP TABLE IF EXISTS mvUserProductSearch;
...
(
    ...
    decShippingCharge           DECIMAL(10,2)   NOT NULL,
...
-- INDEXES
-- ---------------------------------------------------------------------------------
ALTER TABLE tcustomers ADD INDEX tcustomers_customer_NI(strUserName, strCustomerName);
ALTER TABLE tproducts ADD INDEX tproducts_product_NI(intBrandID, intCategoryID);
ALTER TABLE tbrands ADD INDEX tbrands_brand_NI(strBrandTitle);
ALTER TABLE tcategories ADD INDEX tcategories_category_NI(strCategoryTitle);
...
-- ---------------------------------------------------------------------------------
CREATE VIEW VCustomerOrderDeliveryAddresses 
AS
SELECT
	TC.intCustomerID,
    TC.strCustomerName,
...
AND TCO.intCustomerID = TD.intCustomerID
AND TCO.intDeliveryIndex = TD.intDeliveryIndex
AND TD.intCountryID = TY.intCountryID
GROUP BY
	TD.intCustomerID,
    TCO.intOrderIndex,
    TD.intDeliveryIndex;
...
SELECT
    ...
    tp.strProductDescription,
    tp.strProductImage,
    tp.intProductStatusID    
FROM
	tproducts as tp,
    tcategories as tc,
    tbrands as tb
WHERE
	tp.intcategoryid = tc.intcategoryid
...
SELECT
    ...
    IFNULL(dtCOPP.intProductIndex, 0) AS intProductIndex,
	...
	IFNULL(dtCOPP.intQuantity, 0) AS intQuantity,
...
SELECT
	...
	CAST(SUM(COT.intQuantity * COT.decSellingPrice) AS DECIMAL(10,2)) AS decProductTotalPrice,
    (SELECT decShippingCharge 
	 FROM TCustomerOrders 
     WHERE intCustomerID = COT.intCustomerID 
     AND intOrderIndex   = COT.intOrderIndex
    ) AS decShippingCharge,
    (CAST(SUM(COT.intQuantity * COT.decSellingPrice) AS DECIMAL(10,2)) + 
		(SELECT decShippingCharge 
         FROM TCustomerOrders 
         WHERE intCustomerID = COT.intCustomerID AND intOrderIndex = COT.intOrderIndex
		)
    ) AS decOrderTotalPrice    
FROM
...
BEGIN
	...
	START TRANSACTION;	
        ...
        (
			intCustomerID, strIPAddress, strCustomerName, strAddress1, strAddress2, strCity, strState, intCountryID, 
			strZipCode, strUserName, strPassword, strEmailAddress, strPhoneNumber, intCustomerStatusID
		)
		VALUES
        (
			intNewCustomerID, strIPAddress, strCustomerName, strAddress1, strAddress2, strCity, strState, intCountryID, strZipCode, 
			strUserName, strPassword, strEmailAddress, strPhoneNumber, 1
		);
	COMMIT;
	-- Return values to calling program
	SELECT intNewCustomerID AS intCustomerID, strCustomerName as strCustomerName, 
	       strEmailAddress as strEmailAddress;
END
//
...
BEGIN
	START TRANSACTION;    
		...
		SET
			...
			tc.strCustomerName = strCustomerName,
...
BEGIN
	...
	START TRANSACTION;	
        ...
        (
			intProductID, intCategoryID, intBrandID, strProductTitle, decSellingPrice, strProductDescription, 
			strProductImage, dtmCreated, intProductStatusID
		)
		VALUES
        (
			intNewProductID, intCategoryID, intBrandID, strProductTitle, decSellingPrice, strProductDescription, 
			strProductImage, now(), 1
		);
	COMMIT;
...
BEGIN
	START TRANSACTION;
		INSERT INTO tcustomercarts
        (
			intCustomerID, intProductID, intQuantity
		)
        SELECT
			intCustomerID, 
            intProductID, 
...
BEGIN
	...
	START TRANSACTION;	
		-- Get the highest ID and Lock the Table until the end of the transaction
		SELECT 
			MAX( intDeliveryIndex ) + 1 
        INTO 
			intNewDeliveryIndex 
        FROM 
			TCustomerDeliveryAddresses as cda
		WHERE 
			cda.intCustomerID = intCustomerID FOR UPDATE;
		-- Is the ID null (i.e. the Table is empty)?
        ...
        (
			intCustomerID, intDeliveryIndex, strDeliveryName, strAddress1, strAddress2, strCity, strState, 
            intCountryID, strZipCode, intDeliveryAddressStatusID
		)
		VALUES
        (
			intCustomerID, intNewDeliveryIndex, strDeliveryName, strAddress1, strAddress2, strCity, strState, 
...
-- Add Customer Order
CREATE PROCEDURE uspAddCustomerOrders
(
	intCustomerID			INTEGER,
	intDeliveryIndex        INTEGER,
    decShippingCharge		DECIMAL(10,2)
)
BEGIN
	...
	START TRANSACTION;	
		...
		END IF;
		-- Create new record
		INSERT INTO TCustomerOrders
        (
			intCustomerID, intDeliveryIndex, intOrderIndex, dtmOrderDate, decShippingCharge, intCustomerOrderStatusID
		)  							  
		VALUES
        (
			intCustomerID, intDeliveryIndex, intNewOrderIndex, now(), decShippingCharge, 1
		);
	COMMIT;
	-- Return primary key to calling program
...
BEGIN
	DECLARE intNewProductIndex  INTEGER;   
	START TRANSACTION;	
		-- Get the highest Product Index and lock the table until the end of the transaction
		SELECT 
			MAX(intProductIndex) + 1 INTO intNewProductIndex 
		FROM 
			TCustomerOrderItems as tcoi
		WHERE
			tcoi.intCustomerID = intCustomerID
		AND tcoi.intDeliveryIndex = intDeliveryIndex
        AND tcoi.intOrderIndex = intOrderIndex FOR UPDATE;
		
		-- Is the Index null ?
		IF intNewProductIndex IS NULL THEN
			-- Default Index to 1
			SELECT 1 INTO intNewProductIndex;
		END IF;		
        -- Create new record
		INSERT INTO TCustomerOrderItems
        (
			intCustomerID, intDeliveryIndex, intOrderIndex, intProductID, intQuantity, intProductIndex
		)  							  
		VALUES
        (
			intCustomerID, intDeliveryIndex, intOrderIndex, intProductID, intQuantity, intNewProductIndex
		);
	COMMIT;
	-- Return primary key to calling program
	SELECT intCustomerID AS intCustomerID, intDeliveryIndex AS intDeliveryIndex, 
		   intOrderIndex AS intOrderIndex, intProductID AS intProductID;
END
//
...
DELIMITER ;
-- Administrators
INSERT INTO tadministrators 
(
	intAdminID, strAdminName, strUserName, strPassword, strEmailAddress
) 
VALUES
...
-- CustomerStatuses
INSERT INTO tcustomerstatuses 
(
	intCustomerStatusID, strCustomerStatus
)
VALUES
(1, 'Active'),
(2, 'Inactive');
...
VALUES
(1, '::1', 'Hyam Manley', '3400 Northgate Blvd', 'Unit #5', 'Cheviot', 'Ohio', 32, '45222 ', 'hmanley', 'iammanley', 'hmanley@aol.com', '', 1),
...
(29, '::1', 'Dakota Miller', '3951 Delhi Ave', 'Apt #2', 'Cincinnati', 'OH', 32, '45234 ', 'dfmiller', 'pancho54', 'dmiller@gmail.com', '', 1);

--  Statuses
INSERT INTO tdeliveryaddressstatuses 
(
	intDeliveryAddressStatusID, strDeliveryAddressStatus
)
VALUES
(1, 'Primary'),
(2, 'Secondary');
...
(29, 1, 'Dakota Miller', '3964 Delhi Ave', '', 'Cincinnati', 'OH', 32, '45234 ', 1);

-- Brand Statuses
INSERT INTO tbrandstatuses 
(
	intBrandStatusID, strBrandStatus
)
VALUES
(1, 'Active'),
(2, 'Inactive');
...
INSERT INTO tbrands 
(
	intBrandID, strBrandTitle, intBrandStatusID
) 
VALUES
(1, 'HP', 1),
(2, 'Asus', 1),
...
-- Category Statuses
INSERT INTO tcategorystatuses 
(
	intCategoryStatusID, strCategoryStatus
)
VALUES
(1, 'Active'),
(2, 'Inactive');
...
-- Categories
INSERT INTO tcategories 
(
	intCategoryID, strCategoryTitle, intCategoryStatusID
) 
VALUES
(1, 'LapTops', 1),
(2, 'DeskTops', 1),
...
-- Product Statuses
INSERT INTO tproductstatuses 
(
	intProductStatusID, strProductStatus
)
VALUES
(1, 'Active'),
(2, 'Inactive');
...
VALUES
(1, 1, 1, '2020-01-20', 0, 1),
(1, 2, 2, '2020-02-26', 0, 1),
(1, 1, 3, '2020-03-02', 0, 1),
(2, 1, 1, '2020-01-22', 0, 1),
(3, 1, 1, '2020-01-21', 0, 1),
(3, 1, 2, '2020-01-22', 0, 1),
(4, 1, 1, '2020-01-22', 0, 1),
(5, 1, 1, '2020-01-22', 0, 1),
(6, 1, 1, '2020-01-22', 0, 1),
(7, 1, 1, '2020-01-22', 0, 1),
(8, 1, 1, '2020-01-22', 0, 1),
(9, 1, 1, '2020-01-22', 0, 1),
(9, 1, 2, '2020-01-23', 0, 1),
(10, 1, 1, '2020-01-22', 0, 1),
(11, 1, 1, '2020-01-22', 0, 1),
(12, 1, 1, '2020-01-22', 0, 1),
(13, 1, 1, '2020-01-22', 0, 1),
(14, 1, 1, '2020-01-22', 0, 1),
(15, 1, 1, '2020-01-22', 0, 1),
(16, 1, 1, '2020-01-22', 0, 1),
(17, 1, 1, '2020-01-22', 0, 1);
